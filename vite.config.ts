  ...
  return {
    ...
    plugins: [
      react(),
      {
        name: 'copy-manifest',
        writeBundle() {
          // Copy manifest.json to dist (no URL replacement needed since we use <all_urls>)
    ...
    build: {
      ...
      rollupOptions: {
        ...
        output: {
          ...
          manualChunks: (id) => {
            // Inline all dependencies for background and loginMonitor (no separate chunks)
            if (id.includes('background') || id.includes('loginMonitor') || id.includes('constants/env')) {
              return undefined;
            }
            // For other files, allow chunking
            if (id.includes('node_modules')) {
              return 'vendor';
            }
          },
        },
        // External dependencies that should not be bundled (none for service workers)
        external: [],
      },
    },
  };
